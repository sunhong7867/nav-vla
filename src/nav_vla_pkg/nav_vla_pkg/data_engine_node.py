...
Usage:
    ros2 run nav_vla_pkg data_engine_node
    ros2 run nav_vla_pkg data_engine_node --ros-args -p rounds:=5 -p save:=true
    ros2 run nav_vla_pkg data_engine_node --ros-args -p nav_mode:=lane -p rounds:=5
    ros2 run nav_vla_pkg data_engine_node --ros-args -p nav_mode:=lane -p goal_lane:=lane1
    ros2 run nav_vla_pkg data_engine_node --ros-args -p nav_mode:=direct -p rounds:=5
"""
...
DEFAULT_OUT = os.path.expanduser("~/ROS2_project/nav-vla/src/nav_vla_pkg/data")
DIRECT_ONLY_ZONES = {
    "IN",
    "OUT(통과직전)",
    "OUT(통과직후)",
...
class DataEngine(Node):
    ...
    def _wait_for_navigator(self):
        ...
        self.get_logger().error(
            ...
            "Run `navvla` then `ros2 run nav_vla_pkg navigator_node` in another terminal "
