...
lane2 laps first, then lane1 laps. Data is saved under src/nav_vla_pkg/data so it
can be inspected together with the existing Stage-A recordings.

Run:
    python3 src/nav_vla_pkg/scripts/collect_cruise_data.py --laps-per-lane 10
"""
...
class CruiseCollector(Node):
    ...
    def _wait_for_navigator(self):
        ...
        self.get_logger().error(
            ...
            "Run `ros2 run nav_vla_pkg navigator_node` first."
        )
        return False
