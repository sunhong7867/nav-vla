...
class PathVisualizerNode(Node):
    ...
    def visualize_path(self):
        ...
        output_image = self.roi_image.copy()
        height, width = output_image.shape[:2]

        # 경로 점들을 이미지 위에 그리기
        for (x, y) in self.spline_path:
            draw_x = int(max(0, min(width - 1, round(float(x)))))
            draw_y = int(max(0, min(height - 1, height - round(float(y)))))
            cv2.circle(output_image, (draw_x, draw_y), 4, (0, 0, 255), -1)

        # 시각화된 이미지를 ROS 메시지로 변환하여 퍼블리시
        try:
            output_msg = self.cv_bridge.cv2_to_imgmsg(output_image, encoding='bgr8')
