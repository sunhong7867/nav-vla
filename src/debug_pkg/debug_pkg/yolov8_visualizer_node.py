...
class Yolov8VisualizerNode(LifecycleNode):
    ...
    def on_configure(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        # pubs
        self._dbg_pub = self.create_publisher(Image, self.debug_image_topic, 10)
    ...
    def draw_box(self, cv_image: np.array, detection: Detection, color: Tuple[int]) -> np.array:
        ...
        # write text
        id_text = f" {track_id}" if track_id else ""
        label = "{}{} {:.2f}".format(label, id_text, score)
        pos = (min_pt[0] + 5, max(18, min_pt[1] + 22))
        font = cv2.FONT_HERSHEY_SIMPLEX
        (text_w, text_h), baseline = cv2.getTextSize(label, font, 0.55, 2)
        bg_min = (pos[0] - 3, pos[1] - text_h - baseline - 3)
        bg_max = (pos[0] + text_w + 3, pos[1] + baseline + 3)
        cv2.rectangle(cv_image, bg_min, bg_max, color, -1)
    ...
    def draw_mask(self, cv_image: np.array, detection: Detection, color: Tuple[int]) -> np.array:
        ...
        points = [(ele.x, ele.y) for ele in mask_msg.data]
        layer = cv_image.copy()
        if CPFL is not None and CPFL.is_lane_class_name(detection.class_name):
            width = mask_msg.width or cv_image.shape[1]
            height = mask_msg.height or cv_image.shape[0]
            mask_image = CPFL.points_to_mask(points, width, height)
            clean_mask = CPFL.clean_lane_mask(mask_image)
            if clean_mask.shape[:2] != cv_image.shape[:2]:
                clean_mask = cv2.resize(
                    clean_mask,
                    (cv_image.shape[1], cv_image.shape[0]),
                    interpolation=cv2.INTER_NEAREST,
                )
            layer[clean_mask > 0] = color
            cv2.addWeighted(cv_image, 0.4, layer, 0.6, 0, cv_image)
            contours, _ = cv2.findContours(clean_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            cv_image = cv2.drawContours(cv_image, contours, -1, color, 2, lineType=cv2.LINE_AA)
            return cv_image

        mask_array = np.array([[int(ele.x), int(ele.y)] for ele in mask_msg.data], dtype=np.int32)
